...
def get_ordered_columns(players_per_team):
    x  = players_per_team
    ...
    z_zero = ['z_0_pos_x', 'z_0_pos_y', 'z_0_pos_z', 'z_0_rot_x', 'z_0_rot_y', 'z_0_rot_z',
            ...
            'z_0_boost', 'z_0_boost_active', 'z_0_jump_active', 'z_0_double_jump_active', 'z_0_dodge_active', 'z_0_is_demo']
    z_one = []
    z_two = []
    o_zero =[]
    o_one = []
...
def get_structures(proto_game, gdf):
    #Init structures from json
    #Returns goal data and the dataframe
    goal_seconds, goal_frames, goal_scorers, goal_teams = [],[],[],[]
    #Player Data
    z_team_names, o_team_names = [],[]
    #proto_game.teams[0].player_ids
    #proto_game.game_metadata.goals[5].player_id
...
def add_game_columns(gdf, goal_frames, goal_seconds, goal_teams):
    #Add columns for goals predictions, and score, and time until goal
    #Mutates the dataframe
    #Returns the length of the added columns
    ...
    for i in range(len(goal_frames)):
        ...
        arr_secs_remaining = gdf['game_seconds_remaining'][min:goal_frames[i]].to_numpy(copy=True)
        ...
        #until_goal = np.concatenate((until_goal, until_seconds))
        #Update score
...
def reporting(shared, interval_mins):
    try:
        ...
        print("Starting Reporting\n    There are {} total replays.\n    There are {} replays left to process. ({}%)\n".format(in_len, remaining, round(remaining/in_len, 2)*100))
        sys.stdout.flush()
        average = [timedelta(0), 0]
        errors = shared[0].value + shared[1].value + shared[2].value + shared[3].value + shared[4].value + shared[5].value
        start = datetime.now()
        while remaining > 0:
            try:
                ...
                new_errors = shared[0].value + shared[1].value + shared[2].value + shared[3].value + shared[4].value + shared[5].value
                processed = (len(os.listdir(csv_path)) - out_len) + (len(os.listdir(testcsv_path)) - test_len) + (new_errors - errors)
                ...
                print("There are {} replays left to process. ({}%)".format(remaining, round(remaining/in_len,2)*100))
                print("Total errors:{} ({})%".format(errors, round(errors/in_len,2)))
...
def replays_to_csv(in_files, output_path, shared):
    #This is a multiprocessing function called by pre_process_parallel()
    #in_files are the input replays
    #Output path is where the CSV's go
    #verbose_interval is how often you want to print timings
    try:
        count = 1
        ...
        for file in in_files:
            ...
            try:
                ...
                for team in ['z_', 'o_']:
                    for i in range(NUM_PLAYERS):
                        num = str(i) + '_'
                        gen_list = ['pos_x' ,'pos_y', 'pos_z', 'rot_x', 'rot_y', 'rot_z', 'vel_x', 'vel_y', 'vel_z', 'ang_vel_x', 'ang_vel_y',
...
def pre_process_parallel(num_processes, test_ratio = .1, overwrite = False, verbose_interval = 10):
    ...
    #prepare paths
    #if not os.path.exists(os.path.split(replay_path)[0] + "error_replays.txt"):
    #    error_replays = None
    ...
    total_test = len(out_test)
    processes = []
    #TODO: Restructure these loops into 1?
    #Skip existing csv's unless we are overwriting, and count extraneous CSV's
    extraneous = 0
    if overwrite == False:
    ...
    if test_ratio is not 0:
        ...
        if num_test > len(in_files):
            print("There are not enough input files to produce the desired test ratio. This can happen if overwrite = False and we have already partially processed these folders with a smaller ratio.")
            return
        if num_test > 0:
...
#    verbose_interval = float(sys.argv[2])
#    pre_process_parallel(num_processes, test_ratio = .1, overwrite = False, verbose_interval = verbose_interval)
